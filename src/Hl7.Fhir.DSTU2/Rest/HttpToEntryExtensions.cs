        ...
        internal static Resource ParseResource(string bodyText, string contentType, ParserSettings settings, bool throwOnFormatException)
        {
            ...
            try
            {
                if (fhirType == ResourceFormat.Json)
                    result = new FhirJsonParser(DSTU2ModelInfo.Instance, settings).Parse<Resource>(bodyText);
                else
                    result = new FhirXmlParser(DSTU2ModelInfo.Instance, settings).Parse<Resource>(bodyText);
            }
