...
namespace Hl7.Fhir.ElementModel
{
    ...
    {
        public static Base ToPoco(this ISourceNode source, Model.Version version, Type pocoType = null) =>
            new PocoBuilder(version).BuildFrom(source, pocoType);

        public static Base ToPoco(this ISourceNode source, PocoBuilderSettings settings, Type pocoType = null) =>
            new PocoBuilder(settings).BuildFrom(source, pocoType);

        public static T ToPoco<T>(this ISourceNode source, Model.Version version) where T : Base =>
               (T)source.ToPoco(version, typeof(T));

        public static T ToPoco<T>(this ISourceNode source, PocoBuilderSettings settings) where T : Base =>
               (T)source.ToPoco(settings, typeof(T));

        public static Base ToPoco(this ITypedElement element, Model.Version version) =>
            new PocoBuilder(version).BuildFrom(element);

        public static Base ToPoco(this ITypedElement element, PocoBuilderSettings settings) =>
            new PocoBuilder(settings).BuildFrom(element);

        public static T ToPoco<T>(this ITypedElement element, Model.Version version) where T : Base =>
               (T)element.ToPoco(version);
        ...
        public static string ParseResourceReferenceReference(this ITypedElement instance)
        {
