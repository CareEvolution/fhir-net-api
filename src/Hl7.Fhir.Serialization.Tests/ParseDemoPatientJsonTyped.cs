...
namespace Hl7.Fhir.Serialization.Tests
{
    ...
    public class ParseDemoPatientJsonTyped
    {
        ...
        public void IgnoreElements()
        {
            ...
            var jsonBare = patient.ToTypedElement(DSTU2ModelInfo.Instance.StructureDefinitionProvider).ToJson(new FhirJsonSerializationSettings { IgnoreUnknownElements = false });
            Assert.IsTrue(jsonBare.Contains("pat1"));

            patient.Add(SourceNode.Valued("unknownElement", "someValue"));
            var jsonUnknown = patient.ToTypedElement(DSTU2ModelInfo.Instance.StructureDefinitionProvider, settings: new TypedElementSettings { ErrorMode = TypedElementSettings.TypeErrorMode.Ignore }).ToJson(new FhirJsonSerializationSettings { IgnoreUnknownElements = true });
        ...
        public void CatchesSingleValueForArray()
        {
            ...
            var navJson = JsonParsingHelpers.ParseToTypedElement(tp, DSTU2ModelInfo.Instance.StructureDefinitionProvider, null, new FhirJsonParsingSettings() { PermissiveParsing = false });
            var errors = navJson.VisitAndCatch();
            ...
            navJson = JsonParsingHelpers.ParseToTypedElement(tp, DSTU2ModelInfo.Instance.StructureDefinitionProvider, null, new FhirJsonParsingSettings() { PermissiveParsing = false });
            errors = navJson.VisitAndCatch();
