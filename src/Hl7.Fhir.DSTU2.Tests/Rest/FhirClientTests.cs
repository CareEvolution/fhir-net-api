...
namespace Hl7.Fhir.Tests.Rest
{
    ...
    public class FhirClientTests
    {
        ...
        {
            using (var testHandler = new TestHttpHandler())
            using (var client = new FhirClient("http://sqlonfhir-dstu2.azurewebsites.net/fhir", messageHandler: testHandler)) // testEndpoint);
            {
                Bundle result;

                client.CompressRequestBody = true;
                testHandler.BeforeRequest = r =>
                {
                    r.Headers.AcceptEncoding.Clear();
                    r.Headers.AcceptEncoding.Add(new StringWithQualityHeaderValue("gzip"));
                };
                testHandler.AfterResponse = r =>
                {
                    Assert.AreEqual("gzip", r.Content.Headers.ContentEncoding.ToString());
                };
                ...
                testHandler.AfterResponse = null;
                ...
                {
                    r.Headers.AcceptEncoding.Clear();
                    r.Headers.AcceptEncoding.Add(new StringWithQualityHeaderValue("deflate"));
                };
        ...
        public void RequestFullResource()
        {
            ...
            {
                var minimal = false;
                testHandler.BeforeRequest = r => r.Headers.Add("Prefer", minimal ? "return=minimal" : "return=representation");
        ...
        {
            using (var testHandler = new TestHttpHandler())
            using (var client = new FhirClient("http://spark.furore.com/", messageHandler: testHandler)) // an address that returns Status 500 with HTML in its body
            {
                try
                {
                    var pat = client.Read<Patient>("Patient/1");
        ...
        {
            using (var testHandler = new TestHttpHandler())
            using (var client = new FhirClient("https://sqlonfhir.azurewebsites.net/fhir", messageHandler: testHandler))
            {
                testHandler.BeforeRequest = r => r.Headers.Authorization = new AuthenticationHeaderValue("Bearer", "bad-bearer");
                try
                {
        ...
        public void TestCreatingBinaryResource()
        {
            ...
            using (var client = new FhirClient(testEndpoint))
            {
                ...
                Assert.IsNotNull(result);
                testHandler.BeforeRequest = r => r.Headers.Accept.Clear();
